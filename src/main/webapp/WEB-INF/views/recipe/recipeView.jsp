...
/*
	alert("userIdChk : " + userIdChk.typeof);
	
	if (userIdChk.typeof == undefined) 
	{
		
		alert("11111");
		  document.getElementById("btnRecipeDel").style.display = "none";
		  
	} 
	else 
	{
		alert("22222");
		  document.getElementById("btnRecipeDel").style.display = "block";
	}
 ...
 */
   ...
   $("#btnRecipeDel").on("click", function() {
	   if(confirm("레시피를 삭제 하시겠습니까?") == true)
	   {
		$.ajax({
			type:"POST",
			url:"/recipe/recipeDelete",
			data:{
				recipeCode:<c:out value ="${recipeInfo.recipeCode}" />
			},
			datatype:"JSON",
			beforeSend:function(xhr)
			{
			...
			success:function(response)
			{
				...
				{
					alert("댓글이 존재하여 삭제할수 없습니다.");
				}
				else
				{
					alert("레시피 삭제시 오류가 발생하였습니다.");
				}
			},
			      ...
			      <!-- 내가 가진 재료로 만들 수 있는 레시피 리스트들 넣어줄 영역 -->
			        <div class="container-xxl py-5"> 
			            <div class="container">
							...
							<c:if test="${recipeInfo.userId eq cookieUserId}">	
									  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
									 <button id="btnRecipeDel" class="" style="border: 1px solid gray; background-color: #FF9966; color: white; width: 80px; height: 40px; display: inline-block;"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3-fill" viewBox="0 0 16 16">
                             ...
                             <form name="replyForm" id="replyForm" method="post" enctype="multipart/form-data">
                                  ...
                                  <div id="recipeCommentList_1">    
                                    <div class="media reply_list">
                                         ...
                                         <div class="media-left" style="text-align: left;">
                                          ...
                                          <!-- 이미지 옆 닉네임 나오게하기 -->
                                         <span>&nbsp; ${recipeReply.userNickname}</span>
                                          <!-- 이미지 옆 닉네임 나오게하기 -->
                                         ...
                                         <input type="hidden" id = recipeReplySeq name="recipeReplySeq" value="${recipeReply.replySeq}">
                                         <input type="hidden" id = recipeReplyUserId name="recipeReplyUserId" value="${recipeReply.userId}">
                                         
                                         
                                         <!-- 댓글 쓴 유저와 로그인 유저 아이디 체크 -->
                                         <c:if test="${recipeReply.userId eq cookieUserId}">	
                                        	<button type="button" id="btnRecipeDelete" class="btn"><img src="/resources/upload/trash.png" style=width:20px;height:20px;/></button>
                                         </c:if>
                                         </div>
                                         </div>
                              ...
                              <div class="reply_write">
                                  <textarea id="replyContent" name="replyContent" class="" placeholder="댓글을 남겨보세요!" style="height:100px; width:600px; resize:none;"></textarea>
                                     ...
                                     <input type="hidden" id= "replySeq"  name ="replySeq" value="${recipeReply.replySeq}"/> 
                                     <input type="hidden" id= "replyUserId"  name ="replySeq" value="${recipeReply.userId}"/>
                              </div>
 ...
 <!-- 테스트 끝-->
    ...
    <!-- footer 종료 --> 
     ...
     <input type="hidden" name="curPage" value="${curPage}" /> 
   	 ...
   	 <input type="hidden" name ="userId" id= "userId" value="${recipeInfo.userId}"/>
   	 <input type="hidden" id= "replySeq"  name ="replySeq" value="${recipeReply.replySeq}"/> 
 	 <input type="hidden" id= "replyUserId"  name ="replyUserId" value="${recipeReply.userId}"/>
   </form>
</body>
